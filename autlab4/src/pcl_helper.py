...
def ros2pcl(ros_pcloud2):
    """ 
    Convertir un mensaje PointCloud2 en un tipo PCL PointXYZRGB

        Argumentos
            ros_pcloud2 - Mensaje de tipo PointCloud2

        Retorna
            pcl.PointCloud_PointXYZRGB - Nube de puntos XYZRGB de PCL
    """
    points_list = []
    for data in pc2.read_points(ros_pcloud2, skip_nans=True):
...
def pcl2ros(pcl_array):
    """ 
    Convertir un tipo PCL PointXYZRGB en un mensaje PointCloud2 de ROS

        Argumentos
            pcl_array - Nube de puntos de PCL XYZRGB (PointCloud_PointXYZRGB)

        Retorna
            PointCloud2 - Nube de puntos de ROS
    """
    ros_msg = PointCloud2()
...
def XYZRGB_to_XYZ(XYZRGB_cloud):
    """ Convierte una nube de puntos PCL XYZRGB en XYZ (elimina el color)

        Args:
            XYZRGB_cloud (PointCloud_PointXYZRGB): Una nube de puntos PCL XYZRGB

        Retorna:
            PointCloud_PointXYZ: una nube de puntos PCL XYZ
    """
