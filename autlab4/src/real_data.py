...
class Cloud():
    def __init__(self, path):
        self.cloud = pcl.load_XYZRGB(path)
...
cloud = Cloud(os.path.join(os.path.dirname(__file__), '../data/real_data.pcd'))

#cloud.noise_reduction(0.001,2)

# Downsample
cloud.downsample(0.005)

# Pass through
cloud.pass_through( "z", -1, 2)     #no tocar
cloud.pass_through( "y", -0.1, 0.45)    # altura  arriba es mas negativo
cloud.pass_through( "x", -1, 0.3)

# Ransac
cloud.ransac(0.02, False)

# Grabar el resultado en disco
filename = os.path.join(os.path.dirname(__file__), '../data/real_mesa_only.pcd')
